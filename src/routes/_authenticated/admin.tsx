...
function AdminPage() {
  ...
  const uploadImages = async (files: FileList) => {
    ...
    setUploading(false);
    if (succeeded > 0) {
      toast.success(succeeded > 1 ? `${succeeded} images uploaded` : "Image uploaded");
    }
    if (failed > 0) {
      toast.error(
        `${failed} image${failed > 1 ? "s" : ""} failed to upload${lastError ? `: ${lastError}` : ""}`,
      );
    }
  };
                    ...
                    <div key={src + index} className="group relative">
                      ...
                      {index === 0 ? (
                        ...
                        <button
                          ...
                          className="absolute bottom-1 left-1 rounded-none bg-background/90 px-1.5 py-0.5 font-mono text-[9px] uppercase text-muted-foreground"
                        >
                          Make cover
                        </button>
                      )}
