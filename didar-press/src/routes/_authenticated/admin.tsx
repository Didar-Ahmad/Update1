...
function AdminPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (values: FormState) => {
      ...
      if (values.images.length === 0) throw new Error("Upload at least one image for the poster.");
  ...
  const openEdit = (print: PrintRow) => {
    ...
    setForm({
      ...
      images: print.images.length > 0 ? print.images : print.image_url ? [print.image_url] : [],
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          ...
          >
            ...
            <div className="space-y-2">
              <Label htmlFor="image_file">Poster images</Label>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {form.images.map((src, index) => (
                    <div key={src + index} className="group relative">
                      ...
                      />
                      {index === 0 ? (
                        <span className="absolute bottom-1 left-1 rounded-none bg-primary px-1.5 py-0.5 font-mono text-[9px] uppercase text-primary-foreground">
                          Cover
                        </span>
                      ) : (
                        <button
                          ...
                          className="absolute bottom-1 left-1 rounded-none bg-background/90 px-1.5 py-0.5 font-mono text-[9px] uppercase text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100"
                        >
                          Make cover
                        </button>
                      )}
                      <button
                        ...
                        className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-background/90 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100"
                      >
              ...
              <label className="flex cursor-pointer items-center justify-center gap-2 rounded-sm border border-dashed border-border px-3 py-2.5 text-sm text-muted-foreground hover:border-foreground/40">
                <Upload className="h-4 w-4" />
                {uploading ? "Uploading…" : "Upload images (you can select several at once)"}
                <input
                  id="image_file"
                  type="file"
                  accept="image/*"
              ...
              <p className="text-xs text-muted-foreground">
                The first image is used as the cover shown in the shop — hover any other image
                and click "Make cover" to change it.
              </p>
            </div>
            </div>
